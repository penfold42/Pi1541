...
void Screen::UpdateLayout()
{
	...
	if (graphHeight < GetFontHeight()) graphHeight = GetFontHeight();

	topGraphArea = Height()-1-3*graphHeight;
	topAtnGraph = topGraphArea + 1;
	topDatGraph = topGraphArea + 1 + graphHeight;
	topClkGraph = topGraphArea + 1 + 2*graphHeight;

	DriveLedStatusY = topGraphArea - GetFontHeight() * 1;
	DeviceAndRomY = topGraphArea - GetFontHeight() * 2;
}
